...
BC_Type2=read.table(file="https://raw.githubusercontent.com/OscarFHC/IGP_LabExp/master/data/BonC_Type2_20171112.csv", 
                    sep = ",", header = T, fill = T)
...
for (i in 1:nrow(mod.lst)){
  temp <- read.table(file = paste0("https://raw.githubusercontent.com/OscarFHC/IGP_LabExp/master/Analysis/Type2ModResults/Type2_e2_", 
...
dat_mod_II = read.table(file = 
                        paste0("https://raw.githubusercontent.com/OscarFHC/IGP_LabExp/master/Analysis/Type2ModResults/Type2_e2_",
...
ggsave(filename = "D:/Manuscript/IGP_DivEffects_MS_Figs/LabExp_MS_Figs/Ver2/Fig2_Bac_IGP.tiff", 
...
for (i in 1:nrow(mod.lst)){
  tempdat <- read.table(file = paste0("https://raw.githubusercontent.com/OscarFHC/IGP_LabExp/master/Analysis/Type2ModResults/Type2_e2_", 
                                      mod.lst[i, "e2"]*10, "_p_", mod.lst[i,"partition"]*100, "..csv"), sep=",", header=FALSE)
...
ggsave(filename="D:/Manuscript/IGP_DivEffects_MS_Figs/LabExp_MS_Figs/Ver2/Fig2_TypeII_Param.tiff", 
...
###########################################################################################
##### Fig. S2 Comptions vs IGP ############################################################
###########################################################################################
i = which(mod.lst[,"e2"] == e2 & mod.lst[,"partition"] == partition)

temp <- read.table(file = paste0("https://raw.githubusercontent.com/OscarFHC/IGP_LabExp/master/Analysis/Type2ModResults/Type2_e2_", 
                                 mod.lst[i, "e2"]*10, "_p_", mod.lst[i,"partition"]*100, "..csv"), sep=",", header=FALSE)
colnames(temp) <- c("alpha", "R1", "R2", "Cons", "Pred")
consumption <- temp %>% 
  mutate(Pred_Bac = (1.25*(1-partition)*R1 + 1.25*partition*R2) / 
           (1 + 0.8*1.25*(1-partition)*R1 + 0.8*1.25*partition*R2 + 0.38580*0.35959*Cons*alpha),
         Pred_Con = (0.38580*Cons*alpha) / 
           (1 + 0.8*1.25*(1-partition)*R1 + 0.8*1.25*partition*R2 + 0.38580*0.35959*Cons*alpha),
         Con_Bac = (1.25*partition*R1 + 1.25*(1-partition)*R2) / 
           (1 + 0.08*1.25*partition*R1 + 0.08*1.25*(1-partition)*R2))
Comp.L <- consumption %>% 
  select(alpha, Pred_Bac, Pred_Con) %>% 
  gather(key = cons, value = val, Pred_Bac, Pred_Con) %>%
  ggplot() + 
  geom_point(aes(x = alpha, y = val, shape = cons)) + 
  scale_shape_manual(values = c(3, 19), labels = c("Bacteria", "IG prey"), name = "") + 
  scale_x_continuous(limits = c(-0.02, 1.02), expand = c(0, 0)) + 
  scale_y_continuous(limits = c(-0.02, 1.05), expand = c(0, 0)) + 
  labs(x = "",
       y = expression("per capita consumption of IG predator")) + 
  theme_bw() +
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
        ...
        axis.title.y = element_text(size = 18, margin = margin(t = 0, r = 0, b = 0, l = 0)))

Comp <- consumption %>% 
  mutate(TotC = Pred_Bac * Pred + Con_Bac * Cons,
         PredC = Pred_Bac *  Pred,
         ConC = Con_Bac * Cons) %>%
  select(alpha, TotC, PredC, ConC) %>% 
  gather(key = cons, value = val, TotC, PredC, ConC)
Comp$cons <- factor(Comp$cons, levels = c("TotC", "ConC", "PredC"))

Comp.R <- Comp %>%
  ggplot() + 
  geom_point(aes(x = alpha, y = val, shape = cons)) + 
  scale_shape_manual(values = c(3, 1, 19), labels = c("Total", "IG prey", "IG predator"), name = "") + 
  scale_x_continuous(limits = c(-0.02, 1.02), expand = c(0, 0)) + 
  scale_y_continuous(limits = c(1, 5.1), expand = c(0, 0)) + 
  labs(x = "",
       y = expression("Population consumption")) + 
  theme_bw() +
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
        ...
        axis.text = element_text(size = 16), 
        axis.title.y = element_text(size = 18, margin = margin(t = 0, r = 0, b = 0, l = 0)))
FigS2 <- plot_grid(Comp.L, Comp.R, labels = c("a.", "b."), ncol = 2) %>%
  ggdraw() + 
  draw_label("Availability of IG prey (IGP strength)", 
             x = 0.54, y = 0.02, size = 20)
ggsave(plot = FigS2, file = "D:/Manuscript/IGP_DivEffects_MS_Figs/LabExp_MS_Figs/Ver2/FigS2_comptions.tiff",
       width = 36, height = 24, units = c("cm"), dpi = 600)
...
dat <- read.csv(file = "C:/Users/oscar/Downloads/dat.csv", sep = ",", header = TRUE)

bt.f <- function(dat, rep){
  b <- c()
  for (i in 1:rep){
    b <- c(b, mean(sample(dat, length(dat), replace = TRUE)))
  }
  sd(b)
}

dat_Jan <- 1/exp(dat$R_S[which(dat$R_S>0)])
avg_Jan <- mean(dat_Jan)
se_Jan <- bt.f(dat_Jan, 5000)

dat_Ros <- exp(c(-1.128, -0.119, -8.268, -0.364, -0.144, -0.285, -1.374, -4.376, -0.528, 
                 -0.332, -0.0953, -0.165, -1.378, -1.025, -0.092, -0.102, -0.593, -0.0294))

avg_Ros <- mean(dat_Ros)
se_Ros <- bt.f(dat_Ros, 5000)
