...
The model with type I functinoal response explains `r round(R2_pred, 5)*100`
type II functional response can explain `r round(R2_pred, 5)*100` % of the variance
...
ggplot() +
  ...
  geom_line(data=dat.mod_I, aes(x=alpha, y=Res), linetype=3) + 
  labs(x=expression(paste("Contact probability (", alpha, ")")),
       y=expression(paste("% chang to ", alpha, "= 0")), 
       caption=expression("error bars represent standard error of the mean \n dotted line represents model results"))+ 
  #ylim(-0.2, 1.6) + 
